...
export function generateSrtString(blocks: SubtitleBlock[], offsetSec: number = 0): string {
    return blocks.map((block, index) => {
        const start = Math.max(0, block.start + offsetSec);
        const end = Math.max(0, block.end + offsetSec);
        // SRT needs 1-based indexing sequentially
        return `${index + 1}\n${formatTimeForSrt(start)} --> ${formatTimeForSrt(end)}\n${block.text}\n`;
    }).join('\n');
}
